...
class Layer:
    ...
    def __call__(self, x):
        ...
        if self.do_layer_norm:
            # first‐batch lazy init of γ/β
            if self.ln_gain is None:
                self.init_layernorm_params(x)
            # overwrite x with its normalized version
    ...
    def init_layernorm_params(self, x):
        ...
        n_units = x.shape[-1]
        param_shape = [1] * (len(x.shape) - 1) + [n_units]
        self.ln_gain = tf.Variable(tf.ones(param_shape))
        self.ln_bias = tf.Variable(tf.zeros(param_shape))
    ...
    def compute_layer_norm(self, x, eps=0.001):
        ...
        '''
        # compute mean and variance across the feature axis
        mean = tf.reduce_mean(x, axis=-1, keepdims=True)
        variance = tf.reduce_mean(tf.square(x - mean), axis=-1, keepdims=True)
        denom = tf.sqrt(variance) + eps
        x_norm = (x - mean) / denom
